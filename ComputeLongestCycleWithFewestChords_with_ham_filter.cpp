		...
		if (i == lastVertex) 
			{
				...
				if (currentCycleSize > longestCycleSize) {
					...
					if (currentCycleSize == n) {
						//graph is hamiltonian, too many chords
						// printArray(longestCycle,n+1);
						// std::cout << "Graph is Hamiltonian, skipping further search." << std::endl;
						longestCycleSize = n;
						numberOfChords = 10e8;
						return;
					}
				...
				if (currentCycleSize == longestCycleSize){
					int numberOfChords2 = countChords(graph, n, currentCycle, longestCycleSize);
					if (numberOfChords2 < numberOfChords){
						numberOfChords = numberOfChords2;
						std::copy(currentCycle, currentCycle + currentCycleSize, longestCycle);
					}
					// std::cout << numberOfChords2 << ": ";
					// printArray(longestCycle, longestCycleSize);
