#ifndef BITSET_MACROS_HPP
#define BITSET_MACROS_HPP
...
#define bitset_size(set) (__builtin_popcountll(set))

//	Loops over all elements of the set. 
#define forEach(element, set) for (int element = bitset_next((set), -1); (element) != -1; (element) = bitset_next((set), (element)))

//	Loops over all elements of the set starting from start (not included). 
#define forEachAfterIndex(element, set, start) for (int element = bitset_next((set), (start)); (element) != -1; (element) = bitset_next((set), (element)))

//  Returns -1 if the set is empty. Otherwise it executes unsafeNext(set, current).
#define bitset_next(set, current)  (isEmpty(set) ? -1 : unsafeNext((set), (current)) ) //the builtin clz and ctz compiler functions have unexpected behavior at zero.
